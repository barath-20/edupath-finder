...
const CareerPathsSection = () => {
  ...
  const handleExplorePath = (stream: string) => {
    setSelectedStream(stream.toLowerCase());
    setShowVisualizer(true);
  };

  if (showVisualizer && selectedStream) {
    return (
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="bg-card rounded-xl p-6 shadow-card">
            <h2 className="text-2xl font-bold mb-6">
              {selectedStream.charAt(0).toUpperCase() + selectedStream.slice(1)} Career Path
            </h2>
            <CareerPathVisualizer defaultPath={selectedStream} />
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section id="careers" className="py-16 px-4 sm:px-6 lg:px-8 bg-muted/30">
      <div className="container mx-auto max-w-7xl">
            ...
            <Card key={index} className="p-6 border-0 shadow-card hover:shadow-hero transition-all bg-gradient-card">
              <div className="space-y-6">
                ...
                <Button 
                  ...
                  className="w-full" 
                  size="sm"
                  onClick={() => handleExplorePath(stream.stream)}
                >
                  Explore Career Path
        ...
        <div className="bg-card rounded-xl p-8 mt-12 text-center border">
          <h3 className="text-xl font-bold mb-3">Not Sure Which Path to Choose?</h3>
          <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
            Our interactive career path visualizer can help you understand the journey from education to career in different fields.
          </p>
          <Button 
            size="lg" 
            className="bg-gradient-hero hover:shadow-hero transition-all"
            onClick={() => handleExplorePath('engineering')}
          >
